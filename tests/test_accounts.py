...
def test_accounts_crud_and_atomic_balances(auth_client, test_user):
    ...
    res = auth_client.get('/accounts')
    assert res.status_code == 200
    assert b'Archived Accounts' in res.data
    assert b'Pytest Savings Account' in res.data

    # 6. Restore Account
    res = auth_client.post(f'/restore-account/{acc_id}', follow_redirects=True)
    assert res.status_code == 200
...
def test_accounts_user_isolation(client, dual_users):
    acc_b = dual_users['acc_b']
    # Log in as User A
    client.post('/login', data={'email': dual_users['email_a'], 'password': dual_users['pw']}, follow_redirects=True)

    # Attempt to archive User B's account
    res = client.post(f'/toggle-account/{acc_b}', follow_redirects=True)
    assert res.status_code == 200

    with get_db() as (conn, cursor):
        cursor.execute("SELECT is_active FROM accounts WHERE account_id=%s", (acc_b,))
        assert cursor.fetchone()[0] == 1

    # Attempt to delete User B's account
    res = client.post(f'/delete-account/{acc_b}', data={'confirm_text': 'DELETE'}, follow_redirects=True)
    assert res.status_code == 200

    with get_db() as (conn, cursor):
        cursor.execute("SELECT COUNT(*) FROM accounts WHERE account_id=%s", (acc_b,))
        assert cursor.fetchone()[0] == 1
